...
/*
  ...
  * bool - Whether or not the ELM327 was propperly
  initialized
*/
bool ELM327::begin(Stream &stream, const bool& debug, const uint16_t& timeout, const char& protocol)
{
	elm_port    = &stream;
	debugMode   = debug;
	timeout_ms  = timeout;
...
bool ELM327::initializeELM(const char& protocol)
{
	...
	retval=sendCommand(SET_ALL_TO_DEFAULTS);
	if(retval!=ELM_SUCCESS) {connected=false; return connected;}
	delay(100);

	retval=sendCommand(RESET_ALL);
	if(retval!=ELM_SUCCESS) {connected=false; return connected;}
	delay(100);

	retval=sendCommand(ECHO_OFF);
	if(retval!=ELM_SUCCESS) {connected=false; return connected;}
	delay(100);

	retval=sendCommand(PRINTING_SPACES_OFF);
	if(retval!=ELM_SUCCESS) {connected=false; return connected;}
	delay(100);

	retval=sendCommand(ALLOW_LONG_MESSAGES);
	if(retval!=ELM_SUCCESS) {connected=false; return connected;}
	delay(100);
...
float ELM327::ELMVoltage()
{
	status=sendCommand(READ_VOLTAGE);
	payload[3]=0;
	if(status==ELM_SUCCESS)
		return atof(payload)/10.0;
	else
		return 0;	
}
...
/*
  ...
  * float - Actual engine torque in %
*/
float ELM327::fuelRateGramsSecond()
{
	if (queryPID(SERVICE_01, ENGINE_FUEL_RATE_GS))
		return conditionResponse(findResponse(), 4, 1, 0);
...
#ifdef ELM_DEBUG
		Serial.println(F("All chars received: "));
